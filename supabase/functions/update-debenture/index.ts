Deno.serve(async (req) => {
    ...
    try {
        ...
        const { debenture_id, name, description, maturity_date } = await req.json();
        ...
        if (!user || !user.user_roles || user.user_roles.role_name !== 'Global') {
            throw new Error('Acesso negado. Apenas usuários Global podem editar debêntures.');
        }
        ...
        if (maturity_date !== undefined) updateData.maturity_date = maturity_date;
        updateData.updated_at = new Date().toISOString();

        // Atualizar debênture
        const updateResponse = await fetch(`${supabaseUrl}/rest/v1/debentures?id=eq.${debenture_id}`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${serviceRoleKey}`,
                'apikey': serviceRoleKey,
        ...
        if (!updatedDebenture || updatedDebenture.length === 0) {
            throw new Error('Debênture não encontrada ou não foi possível atualizar');
        }
        ...
        try {
            await fetch(`${supabaseUrl}/rest/v1/audit_logs`, {
                ...
                body: JSON.stringify({
                    ...
                    resource_name: name || updatedDebenture[0].name,
                    description: `Debênture atualizada: ${Object.keys(updateData).filter(k => k !== 'updated_at').join(', ')}`,
        ...
        return new Response(JSON.stringify({
            data: updatedDebenture[0],
            message: 'Debênture atualizada com sucesso'
        }), {
