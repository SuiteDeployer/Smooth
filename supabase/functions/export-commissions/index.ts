Deno.serve(async (req) => {
    ...
    try {
        ...
        const csvRows = commissionsData.map((commission, index) => {
            ...
            // Garantir que sempre existe um payment_id válido
            const paymentId = payment?.payment_id || `COM-${commission.id.substring(0, 8).toUpperCase()}-${index + 1}`;
        ...
        // Montar CSV com cabeçalho compatível com importação
        const csvContent = [
            csvHeaders.join(','), // Remove aspas duplas do cabeçalho
        ...
        const totalRecords = commissionsData.length;
        const totalAmount = commissionsData.reduce((sum, commission) => {
            return sum + (commission.monthly_amount || 0);
        }, 0);
