-- Migration: complete_user_management_structure
-- Created at: 1753677381

-- Migration: complete_user_management_structure
-- Completar estrutura para gestão flexível de usuários

-- Tornar company_name obrigatório para usuários de rede (não investidores)
...
BEGIN
    ...
    IF NEW.superior_user_id IS NULL THEN
        SELECT is_top_level INTO STRICT user_role_name
        FROM hierarchy_rules
        WHERE role_name = user_role_name;
        
        IF NOT FOUND OR NOT user_role_name THEN
...
-- Corrigir dados temporariamente para aplicar constraint
UPDATE users SET company_name = 'Empresa Temporária' 
WHERE company_name IS NULL AND user_type = 'network_user';;