-- Migration: fix_users_rls_policy_hierarchy_v2
-- Created at: 1757095346
...
USING (
  ...
  (
    EXISTS (
      ...
      JOIN user_roles current_role ON current_u.role_id = current_role.id
      WHERE current_u.auth_user_id = auth.uid()
      AND current_role.role_name IN ('Master', 'Escritório', 'Assessor')
    )
    AND
