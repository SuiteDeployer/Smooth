...
-- Comentários sobre as correções realizadas:
-- 1. get_all_users_for_global: Adicionado 'WHERE u.status = 'active'' antes do ORDER BY
-- 2. get_user_descendants: 
--    - Caso base: Adicionado 'AND u.status = 'active'' na condição WHERE
--    - Caso recursivo: Adicionado 'AND u.status = 'active'' na condição WHERE
--    - Adicionado ORDER BY para melhorar a apresentação dos resultados
-- 
-- Resultado: Usuários marcados como 'inactive' não serão mais retornados pelas funções RPC,
-- garantindo que a lista seja automaticamente atualizada após deleções.
