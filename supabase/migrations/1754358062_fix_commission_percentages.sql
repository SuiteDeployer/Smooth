-- Migration: fix_commission_percentages
-- Created at: 1754358062

-- Corrigir percentuais de comissão para 4% para todos os papéis
...
BEGIN
    ...
    -- Buscar hierarquia do investidor e criar comissões (CORRIGIDO: 4% para todos)
    FOR v_recipient IN (
        SELECT DISTINCT u.id, u.full_name, ur.role_name as role_name, u.pix, u.pix_key_type,
               CASE ur.role_name
                   WHEN 'Assessor' THEN 4.0
                   WHEN 'Master' THEN 4.0
                   WHEN 'Escritório' THEN 4.0
                   ELSE 0.0
        ...
        IF v_commission_percentage > 0 THEN
            -- Calcular comissão total (4% do valor investido)
            v_total_commission := (NEW.invested_amount * v_commission_percentage / 100);
            
            -- Para exibição no sistema, usamos o valor total da comissão, não dividido por meses
            v_monthly_amount := v_total_commission;
            
            -- Criar apenas um registro de comissão por recipient (não dividir em parcelas mensais)
            ...
            -- Criar pagamento se o schedule foi criado
