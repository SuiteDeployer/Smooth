...
const MasterDashboard = () => {
  ...
  const fetchMasterStats = async () => {
    try {
      ...
      // Buscar escritórios subordinados ao Master
      const { data: escritorios, error: escritoriosError } = await supabase
        .from('users')
        .select(`
          id, 
          full_name, 
      ...
      const { data: investimentos, error: investimentosError } = await supabase
        .from('investments')
        .select('*')
        .in('investor_user_id', todosUsuariosRede)

      if (investimentosError) throw investimentosError

      // Buscar comissões geradas pela rede
