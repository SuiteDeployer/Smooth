...
import { TrendingUp, Users, DollarSign, User, Target, Calendar, Award, CheckCircle, Eye } from 'lucide-react'
...
const AgenteDashboard = () => {
  ...
  // Buscar dados da carteira do agente
  const fetchAgenteStats = async () => {
    try {
      setIsLoading(true)
      setError(null)

      // Buscar clientes (investidores) do agente
      ...
      console.error('Erro ao buscar estatísticas do Agente:', err)
  ...
  return (
    <div className="w-full p-4 md:p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">Visualização da carteira de clientes investidores</p>
        ...
        <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-blue-800 font-semibold">Modo Visualização</h3>
              <p className="text-blue-600 text-sm">Como Agente, você pode visualizar sua carteira e comissões, mas apenas Masters podem criar novos investimentos.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-1">Visualização dos investimentos da sua carteira</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {stats?.performanceInvestimentos?.map((investimento: any, index: number) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{investimento.cliente}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{investimento.serie}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-semibold text-gray-900">{formatCurrency(investimento.valorInvestido)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-semibold text-green-600">{formatCurrency(investimento.valorAtual)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm font-semibold ${investimento.retorno >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatCurrency(investimento.retorno)} ({investimento.percentualRetorno.toFixed(2)}%)
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      investimento.status === 'active' ? 'bg-green-100 text-green-800' :
                      investimento.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{formatDate(investimento.dataInvestimento)}</div>
                  </td>
                </tr>
              ))}
            </tbody>
