...
interface MastersModalProps {
  isOpen: boolean
  onClose: () => void
  data: DashboardMetrics['masters'] | null
}

const MastersModal: React.FC<MastersModalProps> = ({ isOpen, onClose, data }) => {
  if (!data) return null
      ...
      <DialogContent className="max-w-4xl w-[90vw] max-h-[85vh] bg-white border border-gray-200">
        <DialogHeader className="pb-4 border-b border-gray-200">
          ...
          <DialogDescription className="text-gray-500">
            Detalhes dos usuários Master em sua hierarquia
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="overflow-y-auto max-h-[60vh] p-6 space-y-6">
          ...
          {/* Lista de Masters */}
          <div>
            <h3 className="text-lg font-semibold mb-4 text-gray-900">Lista de Masters</h3>
            {data.users.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="mt-2 text-sm text-gray-500">Nenhum Master encontrado na sua rede</p>
              </div>
            )}
          </div>
        </div>
